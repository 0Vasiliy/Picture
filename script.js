...
window.addEventListener('DOMContentLoaded', function () {
  ...
  Object(_modules_checkTextInputs__WEBPACK_IMPORTED_MODULE_4__["default"])('[name="message"]'); // showMoreStyles('.button-styles', '.styles-2'); //Local

  Object(_modules_showMoreStyles__WEBPACK_IMPORTED_MODULE_5__["default"])('.button-styles', '#styles.row'); // Server
...
var accordeon = function accordeon(triggersSelector, itemsSelector) {
  var btns = document.querySelectorAll(triggersSelector),
      blocks = document.querySelectorAll(itemsSelector); //Перебор и анимация аккодеона
...
var burger = function burger(menuSelector, burgerSelector) {
  var menuElem = document.querySelector(menuSelector),
      burgerElem = document.querySelector(burgerSelector); // Сокрытие элементов
...
var calc = function calc(size, material, options, promocode, result) {
  ...
  var calcFunc = function calcFunc() {
    sum = Math.round(+sizeBlock.value * +materialBlock.value + +optionsBlock.value); // Если пользовательне заполнил 1 и 2 блок

    if (sizeBlock.value == '' || materialBlock.value == '') {
      resultBlock.textContent = "Пожалуйста, выберите размер и материал картины"; // Если пользователь запонил 1 и 2 блок то проверяется промокод
...
var checkTextInputs = function checkTextInputs(selector) {
  // Получение элементов
  var txtInputs = document.querySelectorAll(selector); // Перебор элементов
...
var forms = function forms() {
  ...
  var form = document.querySelectorAll('form'),
      ...
      upload = document.querySelectorAll('[name="upload"]'); // Создаём объект с сообщениями
  ...
  upload.forEach(function (item) {
    item.addEventListener('input', function () {
      var dots; // Разбивка строки на имя и после точки

      var arr = item.files[0].name.split('.'); // Проверка на длинну имени
  ...
  form.forEach(function (item) {
    item.addEventListener('submit', function (e) {
      e.preventDefault(); // Блок что показывает пользователю  сообщения

      var statusMessage = document.createElement('div');
      statusMessage.classList.add('status');
      item.parentNode.appendChild(statusMessage); // Форма становится прозрачной

      item.classList.add('animated', 'fadeOutUp'); //  Скрыть форму

      setTimeout(function () {
        item.style.display = 'none';
      }, 400); // Отображение статуса сообщения
      ...
      statusMessage.appendChild(textMessage); // Собираем все данные из введёной формы

      var formData = new FormData(item);
      var api;
      item.closest('.popup-design') || item.classList.contains('calc_form') ? api = path.designer : api = path.question; // Отправляем запрос на сервер
...
var mask = function mask(selector) {
  ...
  function createMask(event) {
    ...
    val = this.value.replace(/\D/g, ''); // Если пользователь пробуеют удалить(+7), то облом
...
var modals = function modals() {
  ...
  function bindModal(triggerSelector, modalSelector, closeSelector) {
    ...
    var trigger = document.querySelectorAll(triggerSelector),
        ...
        scroll = calcScroll(); // Передача в функцию несколько элементов и вешается обработчик
  ...
  function calcScroll() {
    ...
    document.body.appendChild(div); // Вычисление размера прокрутки
...
var pictureSize = function pictureSize(imgSelector) {
  var blocks = document.querySelectorAll(imgSelector); // Функция которая показывает определённый img,когда мышка заходит на img

  function showImg(block) {
    // Получение картинки из блока
    var img = block.querySelector('img'); // Изменеие атрибута src и его замена

    img.src = img.src.slice(0, -4) + '-1.png'; // Скрытие всех лишних элементов(паргафов)
...
var scrolling = function scrolling(upSelector) {
  // Scrolling pageup
  var upElem = document.querySelector(upSelector); // При достижении скрола 1650px,появляется pageup с анимацией
...
/* harmony default export */ __webpack_exports__["default"] = (showMoreStyles); //Local
...
var sliders = function sliders(slides, dir, prev, next) {
  ...
  try {
    var prevBtn = document.querySelector(prev),
        nextBtn = document.querySelector(next); // Если кнопки есть
